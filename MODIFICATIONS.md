...
* Added support to encrypt a given User ID with the certificate of their homeserver.

## 3. Rationale for Modifications

These changes were necessary to prevent a user's homeserver from tracking what remote users they are communicating with. By encrypting the localpart of the invitee's User ID, a user's homeserver knows that they are inviting **some** user from homeserver B, but not which user it is.
...
| `/pkg/hicli/json-commands.go` | Modified the setMembership command for the "invite" action to first encrypt the User ID using h.Client.EncryptUser and then send the invitation using h.Client.InviteUserWithResp with the encrypted ID. |
